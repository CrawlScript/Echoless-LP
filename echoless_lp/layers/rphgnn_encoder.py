...
from echoless_lp.layers.transformer import MyMLP
...
class MultiGroupFusion(nn.Module):
    ...
    def __init__(self,
        ...
        self.group_fc_list = nn.ModuleList([
            ...
            #     ln=i >= num_groups - num_label_groups,
            #     output_ln=i >= num_groups - num_label_groups,
            #     # ln=False,
            #     # output_ln=False,

            # )
            for i, group_input_shape in enumerate(input_shape)
        ])
        ...
        if label_residual:
            self.label_residual_fc = MLP(
                self.global_channels_list,
                input_shape=[-1, sum(group_input_shape[-1] for group_input_shape in input_shape[-num_label_groups:])],
                drop_rate=drop_rate,
                activation=activation,
                output_drop_rate=0.0,
                output_activation=None
    ...
    def forward(self, inputs):
        ...
        if self.label_residual:
            ...
            label_group_h = torch.concat(label_x_list, dim=-1)
            label_h = self.label_residual_fc(label_group_h)
            # import pdb
            # pdb.set_trace()
            h = h + label_h
...
class RpHGNNEncoder(CommonTorchTrainModel):
    ...
    def forward(self, inputs):
        ...
        # dropped_x_group_list = [dropout(x_group) if i < len(x_group_list) - self.num_label_groups else x_group
        #                         for i, (x_group, dropout) in enumerate(zip(x_group_list, self.input_dropouts))]
        ...
        # h_list = [dropout(h) if i >= len(h_list) - self.num_label_groups else h
        #             for i, (h, dropout) in enumerate(zip(h_list, self.input_dropouts))]
