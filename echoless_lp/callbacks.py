...
class CSVLoggingCallback(CallBack):
    ...
    def __init__(self, log_path, extra_logs=None):
        ...
        print("making parent dir: ", parent_dir)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if not (self.model.stop_training or epoch == self.model._epochs - 1):
            return
        ...
        logs_df = logs_df.round(4)

        if not os.path.exists(self.log_path):
            logs_df.to_csv(self.log_path, index=False, header=True)
        else:
            logs_df.to_csv(self.log_path, index=False, header=False, mode="a")
...
class LoggingCallback(CallBack):
    ...
    def __init__(self, log_path, extra_logs=None, use_wandb=False):
        super().__init__()
